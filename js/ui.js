...
import { parseDecimalInput, normalizeDecimalEditValue, normalizeIntegerEditValue } from './utils/numeric-input.js';
import { replaceInputValuePreservingSelection, setInputValidationState } from './utils/input-validation.js';
...
export const dashboardElements = {
    ...
    dailyHubWeeklyProgress: document.getElementById('daily-hub-weekly-progress'),
    dailyHubCurrentStreak: document.getElementById('daily-hub-current-streak'),
    dailyHubBestStreak: document.getElementById('daily-hub-best-streak'),
...
export async function renderSessionView(routine, inProgressData = null) {
    ...
    const updateUserWeightEditingState = () => {
        ...
        if (editResult.isValid) {
            replaceInputValuePreservingSelection(userWeightInput, editResult.value);
            setInputValidationState(userWeightInput, { isValid: true });
            return;
        }

        const result = validateUserWeight(userWeightInput.value);
        setInputValidationState(userWeightInput, {
            isValid: false,
            message: getValidationMessage(result, 'userWeight', {
    ...
    const commitUserWeight = () => {
        const result = validateUserWeight(userWeightInput.value);
        if (result.isValid && result.value !== null) {
            replaceInputValuePreservingSelection(userWeightInput, String(result.value));
            setInputValidationState(userWeightInput, { isValid: true });
            return;
        }
    ...
    userWeightInput.addEventListener('input', updateUserWeightEditingState);
    userWeightInput.addEventListener('blur', commitUserWeight);

    if (inProgressData?.pesoUsuario !== null && inProgressData?.pesoUsuario !== undefined) {
        userWeightInput.value = String(inProgressData.pesoUsuario);
    } else {
        const lastKnownBodyweight = getLastKnownBodyweight(currentUserId);
        if (lastKnownBodyweight !== null) {
            userWeightInput.value = String(lastKnownBodyweight);
        }
    }
    ...
    for (let exerciseIndex = 0; exerciseIndex < routine.exercises.length; exerciseIndex++) {
        ...
        if (exercise.type === 'strength') {
            ...
            const getWeightLimits = allowSignedLoad => ({
                min: allowSignedLoad ? -LIMITS.WEIGHT.max : LIMITS.WEIGHT.min,
                max: LIMITS.WEIGHT.max,
            });

            const updateWeightEditingState = (weightInput, allowSignedLoad, { clearIncomplete = true } = {}) => {
                const editResult = normalizeDecimalEditValue(weightInput.value, {
                    allowSign: allowSignedLoad,
                });

                if (editResult.isValid) {
                    replaceInputValuePreservingSelection(weightInput, editResult.value);
                    if (clearIncomplete || editResult.isComplete) {
                        setInputValidationState(weightInput, { isValid: true });
                    }
                    return;
                }

                const limits = getWeightLimits(allowSignedLoad);
                const result = validateWeight(weightInput.value, {
                    allowSigned: allowSignedLoad,
                    ...limits,
                });
                setInputValidationState(weightInput, {
                    isValid: false,
                    message: getValidationMessage(result, 'weight', limits),
                });
            };

            const commitWeight = (weightInput, allowSignedLoad) => {
                const limits = getWeightLimits(allowSignedLoad);
                const result = validateWeight(weightInput.value, {
                    allowSigned: allowSignedLoad,
                    ...limits,
                ...
                if (result.isValid && result.value !== null) {
                    replaceInputValuePreservingSelection(weightInput, String(result.value));
                    setInputValidationState(weightInput, { isValid: true });
                    return;
                }
            ...
            const updateRepsEditingState = repsInput => {
                const editResult = normalizeIntegerEditValue(repsInput.value);
                if (editResult.isValid) {
                    replaceInputValuePreservingSelection(repsInput, editResult.value);
                    setInputValidationState(repsInput, { isValid: true });
                    return;
                }

                const result = validateReps(repsInput.value);
                setInputValidationState(repsInput, {
                    isValid: false,
                    message: getValidationMessage(result, 'reps', {
                        min: LIMITS.REPS.min,
                        max: LIMITS.REPS.max,
                    }),
                });
            ...
            const commitReps = repsInput => {
                const result = validateReps(repsInput.value);
                if (result.isValid && result.value !== null) {
                    replaceInputValuePreservingSelection(repsInput, String(result.value));
                    setInputValidationState(repsInput, { isValid: true });
                    return;
                }
            ...
            const formatWeightSuggestionPlaceholder = (rawWeight, placeholderType, allowSignedLoad) => {
                ...
                const numericWeight = parseDecimalInput(rawWeight, {
                    allowSign: true,
                    maxFractionDigits: null,
                    roundTo: null,
                }).value;

                if (numericWeight !== null && normalizedPlaceholderType === 'last') {
                ...
                if (numericWeight !== null && normalizedPlaceholderType === 'suggested') {
                    return allowSignedLoad
            ...
            for (let i = 0; i < numberOfSets; i++) {
                ...
                if (suggestions.hasHistory && suggestions.suggestions.lastSets[i]) {
                    ...
                    const lastSetWeight = lastSet
                        ? parseDecimalInput(lastSet.peso, {
                            allowSign: true,
                            maxFractionDigits: null,
                            roundTo: null,
                        }).value
                    ...
                    if (lastSetWeight !== null) {
                        ...
                        suggestionWeight = lastSetWeight;
                        weightInput.dataset.suggestion = String(suggestionWeight);
                    }
                } else {
                    const suggestedWeight = suggestions.hasHistory
                        ? parseDecimalInput(suggestions.suggestions.peso, {
                            allowSign: true,
                            maxFractionDigits: null,
                            roundTo: null,
                        }).value
                    ...
                    if (suggestedWeight !== null) {
                        ...
                        suggestionWeight = suggestedWeight;
                        weightInput.dataset.suggestion = String(suggestionWeight);
                    }
                }
                ...
                weightInput.setAttribute('aria-label', t('session.weight_input_label', { index: i + 1 }));
                weightInput.setAttribute('enterkeyhint', 'next');

                weightInput.addEventListener('input', function () {
                    updateWeightEditingState(weightInput, isBodyweightSelected());
                });

                weightInput.addEventListener('blur', function () {
                    commitWeight(weightInput, isBodyweightSelected());
                });

                if (inProgressExercise?.sets?.[i]?.peso !== null && inProgressExercise?.sets?.[i]?.peso !== undefined) {
                    weightInput.value = String(inProgressExercise.sets[i].peso);
                }
                ...
                repsInput.min = '0';
                repsInput.max = String(LIMITS.REPS.max);
                repsInput.step = '1';
                repsInput.inputMode = 'numeric';
                repsInput.pattern = '[0-9]*';
                repsInput.setAttribute('aria-label', t('session.reps_input_label', { index: i + 1 }));
                repsInput.setAttribute('enterkeyhint', 'next');
                repsInput.addEventListener('input', function () {
                    updateRepsEditingState(repsInput);
                });
                repsInput.addEventListener('blur', function () {
                    commitReps(repsInput);
                });
                if (inProgressExercise?.sets?.[i]?.reps !== null && inProgressExercise?.sets?.[i]?.reps !== undefined) {
                    repsInput.value = String(inProgressExercise.sets[i].reps);
                }
                setRow.appendChild(repsInput);

                if (weightInput.value) {
                    commitWeight(weightInput, initialLoadType === 'bodyweight');
                }
                if (repsInput.value) {
                    commitReps(repsInput);
                }
            ...
            const syncVariantState = () => {
                ...
                weightInputs.forEach((weightInput, setIndex) => {
                    ...
                    const lastSetWeight = lastSetSuggestion
                        ? parseDecimalInput(lastSetSuggestion.peso, {
                            allowSign: true,
                            maxFractionDigits: null,
                            roundTo: null,
                        }).value
                    ...
                    updateWeightEditingState(weightInput, allowSignedLoad, { clearIncomplete: false });
                });
            ...
            if (ex.tipoEjercicio === 'strength' && ex.sets && ex.sets.length > 0) {
                ...
                ex.sets.forEach((set, index) => {
                    ...
                    if (isBodyweightExercise) {
                        const extraLoad = parseDecimalInput(set.peso, {
                            allowSign: true,
                            maxFractionDigits: null,
                            roundTo: null,
                        }).value;
                        const extraLoadText =
                            extraLoad !== null ? `${formatSignedWeight(extraLoad)}` : escapeHtml(set.peso);
                        setContent = t('history.series_bodyweight', {
                            index: index + 1,
                            extraLoad: `${extraLoadText}`,
                        });

                        const totalLoad = parseDecimalInput(set.pesoTotal ?? set.totalWeight, {
                            allowSign: true,
                            maxFractionDigits: null,
                            roundTo: null,
                        }).value;
...
function fillExerciseWithLastValues(exerciseIndex, lastSets) {
    ...
    const allowSignedLoad = normalizeLoadType(exerciseBlock.dataset.loadType) === 'bodyweight';
    ...
    lastSets.forEach((set, setIndex) => {
        ...
        if (weightInput) {
            const weightEdit = normalizeDecimalEditValue(set.peso, { allowSign: allowSignedLoad });
            if (weightEdit.isValid && weightEdit.isComplete) {
                replaceInputValuePreservingSelection(weightInput, weightEdit.value);
                weightInput.dispatchEvent(new Event('input', { bubbles: true }));
            }
        }

        if (repsInput) {
            const repsEdit = normalizeIntegerEditValue(set.reps);
            if (repsEdit.isValid && repsEdit.isComplete) {
                replaceInputValuePreservingSelection(repsInput, repsEdit.value);
