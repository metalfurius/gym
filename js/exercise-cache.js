...
export class ExerciseCacheManager {
    ...
    addExerciseData(
        ...
        const normalizedSets = sets
            .map(set => {
                const weightResult = parseDecimalInput(set.peso, {
                ...
                if (!weightResult.isValid || !repsResult.isValid) {
                    return null;
                }
                ...
                const totalLoadResult = parseDecimalInput(set.pesoTotal ?? set.totalWeight, {
                    allowSign: true,
                    maxFractionDigits: null,
                    roundTo: 1,
                });
                if (totalLoadResult.isValid && totalLoadResult.value !== null) {
                    parsedSet.pesoTotal = totalLoadResult.value;
                }
    ...
    getExerciseSuggestions(exerciseName, executionMode = DEFAULT_EXECUTION_MODE, loadType = DEFAULT_LOAD_TYPE) {
        ...
        const avgReps = Math.round(
            lastSets.reduce((sum, set) => {
                const result = parseIntegerInput(set.reps, { min: 0, max: 1000 });
                return sum + (result.isValid ? (result.value ?? 0) : 0);
            }, 0) / lastSets.length
        );
