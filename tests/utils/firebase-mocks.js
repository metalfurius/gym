...
export const mockFirestore = {
    collection: jest.fn((_db, collectionName) => createMockCollectionRef(collectionName)),
    doc: jest.fn((_db, collectionName, docId) => createMockDocRef(collectionName, docId)),
    addDoc: jest.fn(async (collectionRef, _data) => {
        const docId = `doc-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
        return createMockDocRef(collectionRef.id, docId);
    }),
    setDoc: jest.fn(async (_docRef, _data) => {
        return Promise.resolve();
    }),
    getDoc: jest.fn(async (docRef) => {
        return createMockDocSnapshot(docRef.id, null, false);
    }),
    getDocs: jest.fn(async (_query) => {
        return createMockQuerySnapshot([]);
    }),
    deleteDoc: jest.fn(async (_docRef) => {
        return Promise.resolve();
    }),
    query: jest.fn((...args) => {
        return {
