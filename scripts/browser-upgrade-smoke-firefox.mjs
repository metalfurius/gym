...
async function pageState(browser) {
    return evaluateJson(browser, `(async()=>{const metadata=await fetch('./release.json',{cache:'no-store'}).then(r=>r.json());const bytes=new Uint8Array(await fetch('./js/progress.js',{cache:'no-store'}).then(r=>r.arrayBuffer()));const text=new TextDecoder().decode(bytes).replace(/\\r\\n?/g,'\\n');const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(text));return {revision:document.querySelector('meta[name="gym-release-revision"]')?.content,controller:navigator.serviceWorker.controller?.scriptURL||null,activeWorker:(await navigator.serviceWorker.getRegistration())?.active?.state||null,cacheNames:await caches.keys(),session:localStorage.getItem('${SESSION_KEY}'),metadata,progressHash:Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,'0')).join(''),statusRole:document.getElementById('status')?.getAttribute('role'),statusLive:document.getElementById('status')?.getAttribute('aria-live'),labelFor:document.querySelector('label')?.htmlFor}})()`);
}
